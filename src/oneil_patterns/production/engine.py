...
def analyze_security(
    ...
    The orchestrator composes existing contracts only. It does not introduce
    alternative swing logic or modify detector thresholds.
    """
    if frame.empty:
        return []
    ...
    # P2/P3: morphology-neutral segments -> Flat Base assessments.
    ...
    for segment in segments:
        ...
        records.append(
            _record(
                ...
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_flat_base(flat),
                start=segment.start_date,
                end=segment.end_date,
                confirmed=segment.confirmed_date,
            )
        )
        if flat.state == FlatBaseState.RECOGNIZED:
            ...
            recognized_components.append(
                BaseRegionSummary(
                    ...
                    confirmed_date=segment.confirmed_date,
                    high_price=float(region["high"].max()),
                    low_price=float(region["low"].min()),
                )
            )

    # P4: every confirmed H-L-H-L-H sequence is measured as a DB candidate.
    for i in range(len(candidates) - 4):
        ...
        records.append(
            _record(
                ...
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_double_bottom(assessment),
                start=marks[0].price_date,
                end=marks[3].price_date,
                confirmed=geometry.confirmed_date,
            )
        )

    # P5: shared cup body on every H-L-H; optional immediately-following L-H is
    # interpreted as a handle attempt. Absence remains incomplete, never CWH/CNH.
    for i in range(len(candidates) - 2):
        ...
        records.append(
            _record(
                ...
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_cup_body(body),
                start=body_marks[0].price_date,
                end=body_marks[2].price_date,
                confirmed=geometry.confirmed_date,
            )
        )
            ...
            if [m.type for m in handle_marks] == [LandmarkType.SWING_LOW, LandmarkType.SWING_HIGH]:
                ...
                records.append(
                    _record(
                        ...
                        ticker=ticker,
                        asof_date=asof_date,
                        envelope=normalize_handle(handle_assessment),
                        start=body_marks[2].price_date,
                        end=handle_marks[1].price_date,
                        confirmed=handle_geometry.confirmed_date,
                    )
                )
                family_end = handle_marks[1].price_date
                family_confirmed = handle_geometry.confirmed_date

        family_state = classify_cup_family(
            body,
            handle_assessment,
            right_edge_context_complete=False,
        )
        records.append(
            _record(
                security_id=security_id,
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_cup_family(family_state),
                start=body_marks[0].price_date,
                end=family_end,
                confirmed=family_confirmed,
            )
        )
        if family_state == CupFamilyState.CUP_WITH_HANDLE:
            ...
            recognized_components.append(
                BaseRegionSummary(
                    ...
                    start_date=body_marks[0].price_date,
                    end_date=family_end,
                    confirmed_date=family_confirmed,
                    high_price=float(region["high"].max()),
                    low_price=float(region["low"].min()),
                )
            )

    # P6 Ascending Base: seven alternating turns.
    for i in range(len(candidates) - 6):
        ...
        records.append(
            _record(
                ...
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_ascending_base(assessment),
                start=marks[0].price_date,
    ...
    # P6 Base-on-Base: compose chronological recognized component bases.
    ...
    for first, second in zip(recognized_components, recognized_components[1:]):
        ...
        records.append(
            _record(
                ...
                ticker=ticker,
                asof_date=asof_date,
                envelope=normalize_base_on_base(assessment),
    ...
    # Stable de-duplication protects against equivalent windows reached through
    # overlapping loop paths while preserving distinct semantic states.
