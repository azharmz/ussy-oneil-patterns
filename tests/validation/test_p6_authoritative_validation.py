...
def test_advanced_extractor_rejects_future_bars_before_morphology():
    frame = pd.DataFrame({
        ...
        "close": [10.2, 10.8], "volume": [1000, 1100],
    })
...
def test_base_on_base_components_only_consume_recognized_core_predictions():
    recognized = MorphologyPrediction(candidate_id="a", pattern="FLAT_BASE", start_date=date(2026,1,1), end_date=date(2026,2,1), detector_status="FLAT_BASE_RECOGNIZED")
    ambiguous = MorphologyPrediction(candidate_id="b", pattern="FLAT_BASE", start_date=date(2026,1,1), end_date=date(2026,2,1), detector_status="FLAT_BASE_AMBIGUOUS")
    advanced = MorphologyPrediction(candidate_id="c", pattern="ASCENDING_BASE", start_date=date(2026,1,1), end_date=date(2026,2,1), detector_status="ASCENDING_BASE_RECOGNIZED")
    ...
    assert ADVANCED_PREDICTION_ADAPTER_VERSION == "p6-advanced-prediction-adapter-v0.3"
